...
func StoreArsenal(ArsenalName string, CategoryArsenal string, Description string, Image string) (Response, error) {
	var res Response

	con := db.CreateCon()

	sqlStatement := "INSERT arsenal (arsenalname, categoryarsenal, description, image) VALUES (?, ?, ?, ?)"
	...
	result, err := stmt.Exec(ArsenalName, CategoryArsenal, Description, Image)
	if err != nil {
		return res, err
	}
